			...
			case MemoryOperationMode::Playback:
				if (!this->recording->size()){
					if (!recording_exhausted){
						this->system->get_host()->stdout() << "IO recording has run out!.\n";
...
void MemoryController::use_recording(const char *path, bool record){
	if (record){
		...
		this->system->get_host()->stdout() << "Memory controller is running in recording mode.\n";
	}else{
		std::ifstream file(path, std::ios::binary);
		if (!file){
			this->system->get_host()->stdout() << "MemoryController::use_recording(): ERROR. File not found.\n"
		...
		this->system->get_host()->stdout() << "Memory controller is running in playback mode.\n";
	}
}
#endif
