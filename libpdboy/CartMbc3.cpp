...
Mbc3Cartridge::Mbc3Cartridge(libpdboy &host, std::vector<byte_t> &&buffer, const CartridgeCapabilities &cc):
...
posix_delta_t Mbc3Cartridge::get_rtc_counter_value_ignoring_pause(){
	return this->host->local_now().to_posix() - this->rtc_start_time;
}

posix_delta_t Mbc3Cartridge::get_rtc_counter_value(){
	if (this->rtc_pause_time >= 0)
		return this->host->local_now().to_posix() - this->rtc_pause_time;
...
void Mbc3Cartridge::resume_rtc(){
	auto now = this->host->local_now().to_posix();
	...
	this->host->get_guest().save_rtc(*this, this->rtc_start_time);
}

void Mbc3Cartridge::post_initialization(){
	Mbc1Cartridge::post_initialization();
	this->rtc_start_time = this->host->get_guest().load_rtc(*this);
