...
#define CHECK_FLAG(x) this->system->get_host()->stdout() << (check_flag(this->lcd_control, x) ? " " : "~") << #x "\n"
...
void DisplayController::set_lcd_control(byte_t b){
	...
	return;
	this->system->get_host()->stdout() << "\nLCDC changed: " << std::hex << (int)this->lcd_control << '\n';
...
StdOutEmulator &&operator<<(StdOutEmulator &stream, const RGB &rgb){
	stream << std::hex;
