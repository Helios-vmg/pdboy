...
Cartridge::Cartridge(libpdboy &host): host(&host){
}

Cartridge::~Cartridge(){
}

std::unique_ptr<Cartridge> Cartridge::construct_from_buffer_inner(libpdboy &host, std::vector<byte_t> &&buffer){
...
std::unique_ptr<Cartridge> Cartridge::construct_from_buffer(libpdboy &host, const path_t &path, std::vector<byte_t> &&buffer){
...
StandardCartridge::StandardCartridge(libpdboy &host, std::vector<byte_t> &&buffer, const CartridgeCapabilities &capabilities):
	Cartridge(host){
...
void StandardCartridge::load_ram(){
	...
	if (!this->host->get_guest().load_ram(ram, *this, this->ram.size()))
		return;
	if (ram.size() < this->ram.size())
		return;
