...
class Cartridge{
	...
	static std::unique_ptr<Cartridge> construct_from_buffer_inner(libpdboy &host, std::vector<byte_t> &&);
protected:
	libpdboy *host;
	path_t path;
public:
	Cartridge(libpdboy &host);
	virtual ~Cartridge() = 0;
	static std::unique_ptr<Cartridge> construct_from_buffer(libpdboy &host, const path_t &, std::vector<byte_t> &&);
...
#define DECLARE_UNSUPPORTED_CARTRIDGE_CLASS(x, base) \
	class x : public base{ \
	public: \
		x(libpdboy &host, std::vector<byte_t> &&): base(host){ \
...
#define DECLARE_UNSUPPORTED_STANDARD_CARTRIDGE_CLASS(x) \
	...
	public: \
		x(libpdboy &host, std::vector<byte_t> &&v, const CartridgeCapabilities &cc): StandardCartridge(host, std::move(v), cc){ \
			throw NotImplementedException(); \
		} \
		void write8(main_integer_t, byte_t) override{} \
...
public:
	StandardCartridge(libpdboy &host, std::vector<byte_t> &&, const CartridgeCapabilities &);
